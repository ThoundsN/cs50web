    ...
    socket.on('connect',()=>{
        $('#messageInput').on('keyup' , (key)=>{
          ...
          if (key.keyCode===13 && $(this).val()!="" && !privateWindow && !inRoom) {
            ...
            }//send to general|| broadcast to all
            if (key.keycode === 13 && $(this).val()!= '' && !privateWindow && inRoom){
                const mymessage=$(this).val();
                const username = localStorage.getItem('username')
                ...
                const time = new Date().toDateString()
                $('#messageInput').value = ''
                socket.emit('private',{'username2':username2,'mymessage':mymessage,'username':username,'time':time})
            }

        })

        $("#channelList").on('click','li' ,()=>{
            $('#messageInput').focus()
